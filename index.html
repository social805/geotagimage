...
const drop = $('drop'), fileInput = $('fileInput'), fileList = $('fileList'), processBtn = $('processBtn'), statusEl = $('status'), zipBtn = $('zipBtn');
...
$('jpgBtn').onclick = () => setOutput('jpg'); $('pngBtn').onclick = () => setOutput('png'); $('webpBtn').onclick = () => setOutput('webp');
$('clearBtn').onclick = () => { files=[]; results=[]; renderFiles(); updateButtons(); statusEl.textContent='Đã xoá danh sách.'; zipBtn.disabled=true; };
...
async function processAll(){ if(!validCoords()) return showErr('Toạ độ không hợp lệ.'); results=[]; statusEl.textContent='Đang xử lý...'; processBtn.disabled=true; try{ for(let i=0;i<files.length;i++){ statusEl.textContent=`Đang geotag ${i+1}/${files.length}: ${files[i].name}`; results.push(await processImage(files[i], meta(), output)); } renderResults(); zipBtn.disabled = results.length < 2; }catch(e){ showErr(e.message || String(e)); } finally{ updateButtons(); } }
...
function buildExif(m){ const zeroth={}, exif={}, gps={}; if(m.title) zeroth[piexif.ImageIFD.ImageDescription]=m.title; if(m.author) zeroth[piexif.ImageIFD.Artist]=m.author; if(m.copyright) zeroth[piexif.ImageIFD.Copyright]=m.copyright; if(m.comment) exif[piexif.ExifIFD.UserComment]=m.comment; if(m.date){ const d=m.date.replaceAll('-:',':'); exif[piexif.ExifIFD.DateTimeOriginal]=m.date.replaceAll('-',':')+' 00:00:00'; }
  gps[piexif.GPSIFD.GPSLatitudeRef]=m.lat<0?'S':'N'; gps[piexif.GPSIFD.GPSLatitude]=toDmsRational(Math.abs(m.lat)); gps[piexif.GPSIFD.GPSLongitudeRef]=m.lng<0?'W':'E'; gps[piexif.GPSIFD.GPSLongitude]=toDmsRational(Math.abs(m.lng)); gps[piexif.GPSIFD.GPSMapDatum]='WGS-84';
  if(m.date) gps[piexif.GPSIFD.GPSDateStamp]=m.date.replaceAll('-',':'); return {'0th':zeroth, Exif:exif, GPS:gps, '1st':{}, thumbnail:null}; }
