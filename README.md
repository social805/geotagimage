...
<head>
    ...
    <style>
        /* Tối ưu hóa phông chữ và bố cục Leaflet Map */
        .leaflet-container {
            font-family: inherit;
        }
...
<body class="bg-slate-50 text-slate-800 font-sans">
    ...
    <script type="text/babel">
        ...
        function getOutputFilename(originalName, title, ext) {
            const sanitizedOriginal = originalName.replace(/\.[^.]+$/, "");
            const slugifiedTitle = slugify(title);
            if (slugifiedTitle) {
                return `${slugifiedTitle}-${sanitizedOriginal}.${ext}`;
            }
            return `${sanitizedOriginal}-geotag.${ext}`;
        }
        ...
        // Tải ảnh an toàn hỗ trợ cross-origin nếu chạy trên hosting
        ...
        function GeotagImageTool() {
            ...
            useEffect(() => {
                ...
                try {
                    ...
                    if (!mapInstance.current) {
                        const initialLat = isNaN(currentLat) ? 21.0285 : currentLat;
                        const initialLng = isNaN(currentLng) ? 105.8542 : currentLng;
                        ...
                        mapInstance.current.on('click', (e) => {
                            ...
                            showToast("info", `Đã cập nhật tọa độ mới từ bản đồ: ${clickLat.toFixed(5)}, ${clickLng.toFixed(5)}`);
                        });
                    }

                    if (!isNaN(currentLat) && !isNaN(currentLng) && currentLat >= -90 && currentLat <= 90 && currentLng >= -180 && currentLng <= 180) {
            ...
            // Sửa lỗi kích thước bản đồ khi đổi tab
            useEffect(() => {
                if (activeTab === "location" && mapInstance.current) {
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                            ...
                            <section className="lg:col-span-7 space-y-6">
                                ...
                                <div className="flex bg-white rounded-2xl p-1.5 border border-slate-200 shadow-sm">
                                    {[
                                        { id: "location", label: "Bản Đồ & Tọa Độ Vị Trí", icon: SVG.Map },
